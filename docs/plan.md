...
### 2.1 일감과 소유권의 분리 (G1) — 이 프로젝트의 핵심 규칙

**소유권은 "직접 조작 권한"에만 적용되고, "일감 수행"에는 적용되지 않는다.**

| 작업 종류 | 규칙 |
|---|---|
| **일회성 작업** — 굴착, 건설, 해체, 배송, 청소 | 누구의 복제체나 수행 가능 |
| **건물 운용 작업** — 연구, 요리, 발전기 가동, 운동, 의료 | **소유자의 복제체만** |
| 배타성 | 타일/작업 인스턴스 단위 (ONI 기본 chore 예약 그대로) |
| 충돌 처리 | 선착순. 예약 자리에 남이 먼저 지으면 기존 예약 무산 (ONI 기본 동작) |

**의의**: ONI의 전역 chore 시스템(`GlobalChoreProvider`)을 플레이어별로 쪼갤 필요가 없다.
초판이 "최대 난제"로 지목했던 시뮬레이션 코어 수술이 통째로 사라진다.

**부작용 및 완화** — 구역 연결 후 복제체가 맵 반대편 일감까지 이동한다.
ONI 기본 "근접(Proximity)" 우선순위 + 소유 팟 기준 거리 가중치로 완화하며, 플레이테스트에서 조정한다.
...
**건물 소유자 = 건설 주문을 낸 플레이어.** 실제 시공한 복제체의 소유자와 무관하다.

근거: 2.1에 따라 타 플레이어의 복제체가 내 청사진을 시공할 수 있으므로,
시공자 기준으로 하면 소유자가 비결정적으로 정해진다.
...
| 순위 | 항목 | 비고 |
|---|---|---|
| 1 | 연구 분리 (G2) | `Research` 싱글톤 → 플레이어별 + 건설 메뉴 필터링 |
| 2 | 팟 N개 + 독립 인쇄 타이머 (G4) | `Immigration` 싱글톤 → 팟 단위 |
| 3 | 발신자 컨텍스트 + 권한 계층 | Phase 1·5 |
| 4 | 시작 지점 N개 템플릿 스탬프 (G5) | |
| ~~-~~ | ~~chore 시스템 격리~~ | **G1 결정으로 삭제됨** |
...
| 일감 공유로 인한 복제체 원거리 이동 | 아사·질식 | 근접 우선순위 + 거리 가중치, 플레이테스트 조정 |
